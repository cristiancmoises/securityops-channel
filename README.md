...
| **vaptvupt** | 4.2.0 | built from source (C11 Makefile; source-only since 4.1.0 — vendored SDK `.so`s dropped upstream, links only `-lm -lpthread`; `make check` crypto/security suite runs in-build). 4.2.0: **critical `--dedup` keystream-reuse fix** (re-encrypt `--dedup` archives from ≤ 4.1.0) + pure-PQ `--pq-only` (ML-KEM-768, FIPS 203) | ✅ builds & runs (`vaptvupt`, `zupt`) |
| **vaptvupt-gui** | 4.2.0 | PySide6/Qt6 frontend from the same tarball (GUI now versioned with the CLI); launcher pins the CLI via `VAPTVUPT_BIN` | ✅ builds (`vaptvupt-gui`, `zupt-gui`) |
